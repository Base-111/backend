...
func (r *RedisAuthManager) GetState(ctx context.Context, state string) (string, error) {
	key := r.buildKeyState(state)
...
func (r *RedisAuthManager) DeleteState(ctx context.Context, state string) error {
	key := r.buildKeyState(state)
